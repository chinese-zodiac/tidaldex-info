...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "tokendex") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GET_BLOCKS = (timestamps) => {
  ...
  if (!timestamps || timestamps.length === 0) {
    console.error('GET_BLOCKS: No timestamps provided');
    return gql`query emptyBlocks { blocks(first: 0) { number } }`;
  }

  // Log input timestamps for debugging
  console.log('GET_BLOCKS: Processing timestamps:', timestamps);

  let queryString = 'query blocks {';
  queryString += timestamps.map((timestamp, index) => {
    // Validate timestamp
    if (!timestamp || isNaN(timestamp)) {
      console.error(`GET_BLOCKS: Invalid timestamp at index ${index}:`, timestamp);
      return `invalidTimestamp${index}: blocks(first: 0) { number }`;
    }
  ...
  // Log generated query string for debugging
  console.log('GET_BLOCKS: Generated Query String:', queryString);
...
export const ETH_PRICE = (block) => {
  // Ensure BUNDLE_ID is properly converted to a string
  const bundleId = typeof BUNDLE_ID === 'number'
    ? `"${BUNDLE_ID.toString()}"`
    : `"${BUNDLE_ID}"`
  ...
  const queryString = block
    ? `
    query bundles {
      bundles(where: { id: ${bundleId} }, block: { number: ${Number(block)} }) {
        id
        ethPrice
      }
    }
